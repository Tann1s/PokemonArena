...
public class GamePanelForTournament extends JPanel {
    ...
    private List<String> addedPokemonList = new ArrayList<>();
    private List<Image> addedPokemonImages = new ArrayList<>();

    public GamePanelForTournament(JFrame frame) {
        this.frame = frame;
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (showAddedPlayer && !addedPokemonImages.isEmpty()) {
            int xOffset = 360; // Starting x offset for the first Pokémon image
            for (Image img : addedPokemonImages) {
                g.drawImage(img, xOffset, 200, 60, 60, this); // Draw each added Pokémon image
                xOffset += 70; // Increment offset for next image
            }
        }
    ...
    private JPanel createBattleControlPanel() {
        ...
        addButton.addActionListener(e -> {
            String selectedPokemon = (String) pokemon1.getSelectedItem();
            if (!addedPokemonList.contains(selectedPokemon)) { // Prevent adding duplicates
                addedPokemonList.add(selectedPokemon);
                addedPokemonImages.add(new ImageIcon(pokemonImages.getOrDefault(selectedPokemon, "images/default.png")).getImage());
        ...
        tournamentButton.addActionListener(e -> {
            try {
                showBattle = true;
                if (!battleArena.isPowerOfTwo(addedPokemonList.size())) {
                    throw new RuntimeException("Participant size invalid.");
                }
                performBattle(addedPokemonList);
        ...
        backButton.addActionListener(e -> {
            showBattle = false;
            showAddedPlayer = false;
            addedPokemonList.clear();
            addedPokemonImages.clear();
        ...
        panel.setBounds(400, 590, 560, 30); // Set the position and size of the control panel
        return panel;
    }

    private void printPerState(List<String> results) {
        ...
        new SwingWorker<Void, Void>() {
            protected Void doInBackground() throws InterruptedException {
                ...
                for (String res : results) {
                    if (res.contains("Starting tournament round")) {
                        String[] parts = res.split(" with ");
                        if (parts.length > 1) {
                            String[] pokemons = parts[1].split(" and ");
                            if (pokemons.length > 1) {
                                pokemonOne = pokemons[0].trim();
                                pokemonTwo = pokemons[1].trim();
                                pokemonImage1 = new ImageIcon(pokemonImages.getOrDefault(pokemonOne, "images/default.png")).getImage();
                                pokemonImage2 = new ImageIcon(pokemonImages.getOrDefault(pokemonTwo, "images/default.png")).getImage();
                                repaint();
                            }
                        }
                    }
                    Thread.sleep(100);  // Wait for 1 second
                    String finalPokemonOne = pokemonOne;
                    String finalPokemonTwo = pokemonTwo;
                    SwingUtilities.invokeLater(() -> {
                        battleOutputPane.setText(res);
                        if (res.startsWith(finalPokemonOne)) {
                            if (res.contains("attacking")) {
                                pokemonImage1 = new ImageIcon(pokemonImages.getOrDefault(finalPokemonOne + "_attack", "images/default_attack.png")).getImage();
                            } else if (res.contains("received")) {
                                pokemonImage1 = new ImageIcon(pokemonImages.getOrDefault(finalPokemonOne + "_hurt", "images/default_hurt.png")).getImage();
                            } else if (res.contains("defend")) {
                                pokemonImage1 = new ImageIcon(pokemonImages.getOrDefault(finalPokemonOne + "_defend", "images/default_defend.png")).getImage();
                            } else if (res.contains("lost")) {
                                pokemonImage1 = new ImageIcon(pokemonImages.getOrDefault(finalPokemonOne + "_lose", "images/default_lose.png")).getImage();
                            } else if (res.contains("won")) {
                                pokemonImage1 = new ImageIcon(pokemonImages.getOrDefault(finalPokemonOne + "_win", "images/default_win.png")).getImage();
                            }
                        }
                        if (res.startsWith(finalPokemonTwo)) {
                            if (res.contains("attacking")) {
                                pokemonImage2 = new ImageIcon(pokemonImages.getOrDefault(finalPokemonTwo + "_attack", "images/default_attack.png")).getImage();
                            } else if (res.contains("received")) {
                                pokemonImage2 = new ImageIcon(pokemonImages.getOrDefault(finalPokemonTwo + "_hurt", "images/default_hurt.png")).getImage();
                            } else if (res.contains("defend")) {
                                pokemonImage2 = new ImageIcon(pokemonImages.getOrDefault(finalPokemonTwo + "_defend", "images/default_defend.png")).getImage();
                            } else if (res.contains("won")) {
                                pokemonImage2 = new ImageIcon(pokemonImages.getOrDefault(finalPokemonTwo + "_win", "images/default_win.png")).getImage();
                            } else if (res.contains("lost")) {
                                pokemonImage2 = new ImageIcon(pokemonImages.getOrDefault(finalPokemonTwo + "_lose", "images/default_lose.png")).getImage();
                            }
                        }
                        repaint();
                    });
    ...
    private void performBattle(List<String> pokemons) throws BattleLostException, ClassNotFoundException {
        PrintStream originalStdOut = System.out;
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        System.setOut(new PrintStream(outputStream));
        List<String> outputString = new ArrayList<>();
        battleArena.tournament(pokemons);
